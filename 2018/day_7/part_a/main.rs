...
/*
 * Keep a Vec<char> `to_execute` of executable nodes.
 * Keep a HashSet<char> `executed` of executed nodes
 * - add all nodes without parents
 * - while to_execute not empty:
 *     - sort to_execute
 *     - pop off (execute) first node `n`; add to executed
 *     - for each child `c` of `n`:
 *        - if all parents of c are in executed, add c to to_execute
 */
