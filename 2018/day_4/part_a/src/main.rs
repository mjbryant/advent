...
fn get_sleepiest_guard(sleep_times_by_guard: &HashMap<i32, Vec<u32>>) -> i32 {
    let mut max_sleep = 0;
    let mut max_guard = 0;
    for (guard_id, minutes) in sleep_times_by_guard {
        let sleep: u32 = minutes.iter().sum();
        if sleep > max_sleep {
            max_guard = *guard_id;
            max_sleep = sleep;
        }
...
fn get_sleepiest_minute(guard_id: i32, sleep_times_by_guard: &HashMap<i32, Vec<u32>>) -> u32 {
    let times = sleep_times_by_guard.get(&guard_id).unwrap();
    let mut max_sleep = 0;
    let mut max_minute = 0;
    for i in 0..times.len() {
        if times[i] >= max_sleep {
            max_sleep = times[i];
            max_minute = i;
        }
    }
...
fn main() {
    ...
    let guard_id = get_sleepiest_guard(&sleep_times_by_guard);
    let sleepiest_minute = get_sleepiest_minute(guard_id, &sleep_times_by_guard);
    println!(
        "Sleepiest guard: {:?} (slept the most on minute {:?}); Checksum: {:?}",
        guard_id,
        sleepiest_minute,
        guard_id * sleepiest_minute as i32,
    );
}
