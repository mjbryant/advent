 ...
 * entire string until there are no more reactions.
 *
 * Approach: keep a pointer to a reagent. Try to react it with the reagent
 * before it in the string. If it works, remove both of those and decrement the
 * pointer. If not, advance the pointer.
 */
fn main() {
    ...
    react_polymer(&mut b);
    println!("Ended with a polymer of length {:?}", b.len());
}
